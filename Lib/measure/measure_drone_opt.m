...
function [y, obs] = measure_drone_opt(x,params,t,u,obs)
    ...
    tmp = reshape(x(params.pos_obs,:),size(x(params.pos_obs,:),1),size(x(params.pos_obs,:),2));
    y = [y;tmp];%.*ones(length(params.pos_obs),size(y,2))];
    noise = obs.init.noise_story(traj).val(:,pos);
    obs.init.Ytrue_full_story(traj).val(1,:,pos) = y;
    y = y + noise;
