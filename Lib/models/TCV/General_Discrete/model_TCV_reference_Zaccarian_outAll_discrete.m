...
function x_dot = model_TCV_reference_Zaccarian_outAll_discrete(t,x,params,obs)
    ...
    x_dot(range,:) = params.discrete*x_dot(range,:) + params.Ar*x(range,:) + params.Br*0; 
    ...
    x_dot(range,:) = params.discrete*x_dot(range,:) + params.Ac*x(range,:) + params.Bc*e + params.Bcr*rbar; 
    ...
    x_dot(range,:) = params.discrete*x_dot(range,:) + params.sys_pert(obs.init.traj).A*x(range,:) + params.sys_pert(obs.init.traj).B*(u);  
