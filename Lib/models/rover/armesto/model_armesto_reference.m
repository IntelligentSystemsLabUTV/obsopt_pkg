...
function [x_dot, x] = model_armesto_reference(tspan,x,params,obs)
    ...
    x_dot(params.pos_bias) = x_dot(params.pos_bias) + params.Ts*x(params.pos_bias_vel);        
    ...
    % noise dynamics
    x_dot(params.pos_jerk) = 0*x(params.pos_jerk) + params.jerk_enable*params.Q(1:3,1:3)*ones(3,1)*params.Ts*chirp(tspan(1),1,5,100)*cos(2*tspan(1))^2;
    x_dot(params.pos_alpha) = 0*x(params.pos_alpha) + params.alpha_enable*params.Q(4:6,4:6)*ones(3,1)*params.Ts*chirp(tspan(1),1,5,100)*1e1*sin(2*tspan(1))^2;  
    x_dot(params.pos_bias_vel) = x(params.pos_bias_vel) + params.bias_vel_enable*params.Q(7:9,7:9)*params.Ts*ones(3,1)*sin(tspan(1));
