...
function x_dot = model_rover(tspan,x,params,obs)
    ...
    % model dynamics
    % x axis
    x_dot(1) = x(2) + params.L(1)*params.G(1)*(x(5)-x(1)) + params.alpha(1)*x(1);
    x_dot(2) = x(3) + params.L(1)^2*params.G(2)*(x(5)-x(1)) + params.alpha(2)*x(2); 
    x_dot(3) = x(4) + params.K(1)*params.C(1)*(a(1)-x(3));
    x_dot(4) = params.K(1)^2*params.C(2)*(a(1)-x(3));
    x_dot(5) = 0;    

    % y axis
    x_dot(6) = x(7) + params.L(2)*params.G(3)*(x(10)-x(6)) + params.alpha(3)*x(6);
    x_dot(7) = x(8) + params.L(2)^2*params.G(4)*(x(10)-x(6)) + params.alpha(4)*x(7);
