...
classdef obsopt_v1
    ...
    methods
        % constructor
        function obj = obsopt_v1(Ts,Nw,Nts)
            ...
            % model plant
            obj.setup.model = @runaway_model;
            obj.setup.measure = @measure;
        ...
        function y_read = measure_function(obj,varargin)
            ...
            % fixed sampling
            n_iter = (W_buf_pos)*obj.setup.Nts-1;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if obj.setup.forward
                ...
                for i=1:n_iter
                    ...
                    % integration
                    X = obj.setup.ode(@(t,x)obj.setup.model(t, x_propagate, params), obj.setup.tspan, x_propagate);
                    x_propagate = X.y(:,end);
            ...
            % get the measure
            y_read = obj.setup.measure(x_propagate);
