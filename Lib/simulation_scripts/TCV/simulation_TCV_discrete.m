...
% INPUT: none
% OUTPUT: params,obs
function [params,obs] = simulation_TCV_discrete(dim,sys_P,staticAll,discrete)
...
params_init = @params_TCV_Zaccarian_outAll;
...
params_update = @params_update_TCV_Zaccarian_outAll;
...
model = @model_TCV_Zaccarian_outAll;
...
params = model_init('Ts',Ts,'T0',[t0, tend],'noise',0, 'params_update', params_update, ...
            ...
            'input_enable',1,'input_law',input_law,'params_init',params_init,'params_init_vars',{dim,sys_P,staticAll,discrete,Ts});
...
for i = 1:obs.setup.Niter
    ...
    for traj = 1:obs.setup.Ntraj
        ...
        obs.init.Yref_full_story(traj).val(1,:,obs.init.ActualTimeIndex) = obs.setup.measure_reference(obs.init.X(traj).val(:,obs.init.ActualTimeIndex),obs.init.params,obs.setup.time(obs.init.ActualTimeIndex),...
                                                                            obs.init.input_story_ref(traj).val(:,max(1,obs.init.ActualTimeIndex-1)));
        obs.init.noise_story(traj).val(:,obs.init.ActualTimeIndex) = zeros(length(obs.setup.observed_state),1);
