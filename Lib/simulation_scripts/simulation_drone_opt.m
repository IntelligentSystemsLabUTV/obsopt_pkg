...
% INPUT: none
% OUTPUT: params,obs
function [obs,params] = simulation_drone_opt
...
%%%% measure function %%%%
measure_reference = @measure_drone_reference_opt;
measure = @measure_drone_opt;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
obs = obsopt('DataType', 'simulated', 'optimise',1 , 'print', 1, ... 
          ...
          'FlushBuffer', 1, ...
          'opt', @fminsearchcon, ... %@fminunc @fminsearchcon @patternsearch
          'Jdot_thresh',0.95, 'MaxIter', 50, 'Jterm_store', 1, 'AlwaysOpt', 0 , 'SafetyDensity', Inf, ...
          'MultiStart', params.multistart, 'J_normalise', 0, 'MaxOptTime', Inf, ...
    ...
    if 1
        ...
        if(obs.init.ActualTimeIndex > 1)
            for traj=1:params.Ntraj
                tmp =  reshape(obs.init.noise_story(traj).val(:,:),size(obs.init.noise_story(traj).val,2),params.OutDim);
                obs.init.Yhat_full_story(traj).val(1,:,:) = squeeze(obs.init.Ytrue_full_story(traj).val(1,:,:)) + obs.init.noise_story(traj).val(:,:);
            end
        end
