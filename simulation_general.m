...
measure = @pendulum_measure;
ode = @oderk4;

params = pendulum_init(Ts,t0,tend,'noise',1,'model',model,'measure',measure,'StateDim',2,'ObservedState',2,'ode',ode);

% init observer
Nw = 5;
NTs = 3;

obs = obsopt_general(Ts,Nw,NTs,'model',model,'measure',measure,'ode',ode,'print',0,'MaxIter',500);
...
for i = 1:params.Niter
    ...
    %%%%%%%%%%%%%%%%%%% REAL MEASUREMENT %%%%%%%%%%%%%%%%%%%%%%%    
    params.Y(:,params.iter) = params.measure(params.X(:,params.iter),params) + params.noise*(0  + 1e-1*randn);
